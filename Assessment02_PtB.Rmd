...
```{r lm_log, echo=TRUE, warning=FALSE}
library(ggplot2)
library(ggfortify)
...
```
```{r lm_log_4plots, echo=TRUE, warning=FALSE}
autoplot(fit1)
```
...
trainset_size <- floor(0.7 * nrow(df_selected))
validation_size <- floor(0.1 * nrow(df_selected))
testset_size <- floor(0.2 * nrow(df_selected))
trainset_indices <- sample(seq_len(nrow(df_selected)), size = trainset_size)
trainset <- df_selected[trainset_indices, ]
df_set <- df_selected[-trainset_indices, ]
...
```{r SLR_logxlogy_modelling}
lm_ETH = lm(formula = log(BTC.USD.Close) ~ log(ETH_TDlag_1), data = trainset)
summary(lm_ETH)

testset_selected <- subset(testset, select=c(ETH_TDlag_1,
                                             BTC.USD.Close))
ggplot(trainset, aes(x=log(ETH_TDlag_1), y=log(BTC.USD.Close))) + 
  geom_point() + 
...
# save 2021 Sep and Octo for prediction
...
car::vif(besttune_lm$finalModel) #VIF BTC greater than 10
